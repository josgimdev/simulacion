...
class PlaneSection
{
   ...
   {
      float s1 = PVector.sub(_pos1, p).mag();
      float s2 = PVector.sub(_pos2, p).mag();
      float distanciaPlano = PVector.sub(_pos2, _pos1).mag();

      if (s1 >= distanciaPlano || s2 >= distanciaPlano)
         return false;
      else
         return true;
   }
